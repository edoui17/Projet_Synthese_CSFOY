...
3. **StateMachine** : La logique pure C# dans la couche Core/Logic, gérant les états `IDLE`, `CHASE` et le timer de désengagement.
...
2. **LineOfSightRay (RayCast2D)** :
   ...
   - Assurez-vous qu'**aucun script** (comme `StateMachine.cs`) n'est attaché à ce nœud dans l'éditeur.
...
### 3. Changement d'État
Le `StateMachine` reçoit ce booléen.
