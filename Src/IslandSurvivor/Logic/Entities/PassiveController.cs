...
public class PassiveController
{
    ...
    public void Update(float p_delta)
    {
        ...
        if (m_currentState == NpcStates.FLEE)
        {
            m_fleeTimer -= p_delta;
            if (m_fleeTimer <= 0)
            {
    ...
    public void StartFleeing(Vector2 p_npcPosition, Vector2 p_attackerPosition)
    {
        m_currentState = NpcStates.FLEE;
        m_fleeTimer = FLEE_DURATION;

        // Calculate direction opposite to attacker
        Vector2 direction = p_npcPosition - p_attackerPosition;
