...
- **AI Contribution:** Designed and implemented a `WeakEvent` pattern in C# (Core) using `WeakReference` to allow the Garbage Collector to automatically clean up destroyed Godot Nodes. Created `ISignalManager` and its implementations. Wrote xUnit tests to validate the weak references. Avoided `var` keywords and only put a commented example in the interface.
- **Decision Reasoning:** Chose pure C# events with `WeakReference` over native Godot signals to decouple the `Core` logic from the Godot Engine and allow thorough unit testing of the communication architecture without spinning up the Godot environment.