...
- **Component Delegation**: When implementing new movement states like a `Dash` (burst speed, non-interruptible), encapsulate the timers and velocity execution directly into a generic movement component (e.g., `MovementController`) rather than the `Player.cs` controller. This allows both players and enemies to easily invoke `TryDash()` in the future without duplicating state timers.
